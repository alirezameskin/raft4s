...
class KvStateMachine extends StateMachine {
  ...
  override def applyWrite: PartialFunction[(Long, WriteCommand[_]), Any] = { case (index, Put(key, value)) =>
    map.put(key, value)
    lastIndex = index
    value
  }

  override def applyRead: PartialFunction[ReadCommand[_], Any] = { case Get(key) =>
    map.get(key)
  }
}
