...
class KvStateMachine extends StateMachine {
  private var map = new HashMap[String, String]()
  private var lastIndex: Long = 0

  override def applyWrite: PartialFunction[(Long, WriteCommand[_]), Any] = {
    case (index, Put(key, value)) =>
      map.put(key, value)
      lastIndex = index
      value
  }

  override def applyRead: PartialFunction[ReadCommand[_], Any] = {
    case Get(key) =>
      map.get(key)
  }
}
...
object RaftTest extends App {
  ...
  val log2 = new MemoryLog()
  val stateMachine2 = new KvStateMachine()
  val rlog2 = new ReplicatedLog(log2, stateMachine2)
  val raftNode2 = new Raft("node2",Map.empty, rlog2, Ref.of[IO, NodeState](FollowerNode("node2", nodes, 0, None, None)).unsafeRunSync())
  val raft2Client = new RpcClient {
  ...
  val log3 = new MemoryLog()
  val stateMachine3 = new KvStateMachine()
  val rlog3 = new ReplicatedLog(log3, stateMachine3)
  val raftNode3 = new Raft("node3",Map.empty, rlog3, Ref.of[IO, NodeState](FollowerNode("node3", nodes, 0, None, None)).unsafeRunSync())
  val raft3Client = new RpcClient {
    override def send(voteRequest: VoteRequest): IO[VoteResponse] = {
