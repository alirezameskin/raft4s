...
case class FollowerNode(nodeId: String, nodes: List[String], currentTerm: Long, votedFor: Option[String] = None, currentLeader: Option[String] = None) extends NodeState {
  ...
  override def onReceive(logState: LogState, msg: VoteRequest): (NodeState, VoteResponse) = {
    ...
    val termOK = (msg.currentTerm > currentTerm) || (msg.currentTerm == currentTerm && (votedFor.isEmpty || votedFor.contains(msg.nodeId)))
  ...
  override def onTimer(logState: LogState): (NodeState, List[Action]) = {
    ...
    ( CandidateNode(nodeId, nodes, currentTerm + 1, lastTerm, Some(nodeId), Set(nodeId))
